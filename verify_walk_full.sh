...
# Walk + verify + auto-fix + write canonical contents (idempotent, backups if changed)
set -euo pipefail
ROOT=~/meta-spiratech
cd "$ROOT"
...
EOT
write_file "packages/meta-core/src/metaLoop.ts" "/tmp/metaLoop.ts"

# 2) packages/meta-core/src/skills.ts
cat > /tmp/skills.ts <<'EOT'
...
EOT
write_file "packages/meta-core/src/skills.ts" "/tmp/skills.ts"

# 3) packages/connectors/src/index.ts
cat > /tmp/connectors.ts <<'EOT'
...
EOT
write_file "packages/connectors/src/index.ts" "/tmp/connectors.ts"

# 4) apps/pulseo-web/app/providers.tsx
cat > /tmp/providers.tsx <<'EOT'
"use client";
...
export default function Providers({ children }: { children: React.ReactNode }) {
  // fallback si AutoCoreProvider indispo
  const AC:any = AutoCoreProvider ?? ((p:any)=>p.children);
  return <AC><MetaBridge/>{children}</AC>;
}
EOT
write_file "apps/pulseo-web/app/providers.tsx" "/tmp/providers.tsx"

# 5) apps/pulseo-web/app/api/rss/route.ts
cat > /tmp/rss_route.ts <<'EOT'
...
EOT
write_file "apps/pulseo-web/app/api/rss/route.ts" "/tmp/rss_route.ts"

# 6) public/manifest.json
cat > /tmp/manifest.json <<'EOT'
{
  "name": "Pulseo",
  "short_name": "Pulseo",
  "start_url": "/",
...
EOT
write_file "public/manifest.json" "/tmp/manifest.json"
...
EOT
write_file "public/sw.js" "/tmp/sw.js"

# 8) config/meta.json (merge minimal si existe)
if [ ! -f "config/meta.json" ]; then
  cat > /tmp/meta.json <<'EOT'
{
...
EOT
  write_file "config/meta.json" "/tmp/meta.json"
else
  say "OK  config/meta.json (exists) — conserve"
fi

# 9) .gitignore minimal
cat > /tmp/gitignore <<'EOT'
node_modules
.env*
*.log
coverage
...
dist
EOT
write_file ".gitignore" "/tmp/gitignore"

# 10) nettoyage fichiers parasites
find . -type f \( -name "*~" -o -name "*.bak~" -o -name "*.tmp" \) -print -delete

say "✔️  Walk+Fix terminé."
